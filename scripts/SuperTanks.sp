...
public OnPluginStart()
{
	...
	hWave3Cvar = CreateConVar("st_wave3_tanks", "3", "Default number of tanks in the finale escape.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,5.0);
	hFinaleOnly = CreateConVar("st_finale_only", "1", "Create Super Tanks in finale only?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hSpawnExtraHealth = CreateConVar("st_spawn_extra_health", "0", "Spawn Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hSpawnFireImmunity = CreateConVar("st_spawn_fire_immunity", "1", "Is Spawn Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hSpawnCommonAmount = CreateConVar("st_spawn_common_amount", "10", "Number of common infected spawned by the Spawn Tank.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,1.0,true,50.0);
	hSpawnCommonInterval = CreateConVar("st_spawn_common_interval", "10", "Spawn Tanks common infected spawn interval.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,1.0,true,60.0);

	hSmasherEnabled = CreateConVar("st_smasher", "1", "Is Smasher Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hSmasherExtraHealth = CreateConVar("st_smasher_extra_health", "0", "Smasher Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	hSmasherSpeed = CreateConVar("st_smasher_speed", "0.65", "Smasher Tanks default movement speed.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,2.0);
	...
	hSmasherCrushDamage = CreateConVar("st_smasher_crush_damage", "300", "Smasher Tanks claw attack damage.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hSmasherRemoveBody = CreateConVar("st_smasher_remove_body", "1", "Smasher Tanks crush attack will remove survivors death body?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);

	hWarpEnabled = CreateConVar("st_warp", "1", "Is Warp Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hWarpExtraHealth = CreateConVar("st_warp_extra_health", "0", "Warp Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hWarpFireImmunity = CreateConVar("st_warp_fire_immunity", "1", "Is Warp Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hWarpTeleportDelay = CreateConVar("st_warp_teleport_delay", "20", "Warp Tanks Teleport Delay Interval.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,1.0,true,60.0);

	hMeteorEnabled = CreateConVar("st_meteor", "1", "Is Meteor Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hMeteorExtraHealth = CreateConVar("st_meteor_extra_health", "0", "Meteor Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hMeteorFireImmunity = CreateConVar("st_meteor_fire_immunity", "1", "Is Meteor Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hMeteorStormDamage = CreateConVar("st_meteor_storm_damage", "25.0", "Meteor Tanks falling meteor damage.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);

	hSpitterEnabled = CreateConVar("st_spitter", "1", "Is Spitter Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hHealExtraHealth = CreateConVar("st_heal_extra_health", "0", "Heal Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hHealFireImmunity = CreateConVar("st_heal_fire_immunity", "1", "Is Heal Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hHealHealthCommons = CreateConVar("st_heal_health_commons", "10", "Heal Tanks receive this much health per second from being near a common infected.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthSpecials = CreateConVar("st_heal_health_specials", "100", "Heal Tanks receive this much health per second from being near a special infected.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthTanks = CreateConVar("st_heal_health_tanks", "500", "Heal Tanks receive this much health per second from being near another tank.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);

	hFireEnabled = CreateConVar("st_fire", "1", "Is Fire Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hFireExtraHealth = CreateConVar("st_fire_extra_health", "0", "Fire Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hIceEnabled = CreateConVar("st_ice", "1", "Is Ice Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hJockeyEnabled = CreateConVar("st_jockey", "1", "Is Jockey Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hJockeyExtraHealth = CreateConVar("st_jockey_extra_health", "0", "Jockey Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hGhostExtraHealth = CreateConVar("st_ghost_extra_health", "0", "Ghost Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hGhostFireImmunity = CreateConVar("st_ghost_fire_immunity", "1", "Is Ghost Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hGhostDisarm = CreateConVar("st_ghost_disarm", "1", "Does Ghost Tank have a chance of disarming an attacking melee survivor?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);

	hShockEnabled = CreateConVar("st_shock", "1", "Is Shock Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hShockExtraHealth = CreateConVar("st_shock_extra_health", "0", "Shock Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hShockFireImmunity = CreateConVar("st_shock_fire_immunity", "1", "Is Shock Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hShockStunDamage = CreateConVar("st_shock_stun_damage", "12", "Shock Tanks stun damage.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hShockStunMovement = CreateConVar("st_shock_stun_movement", "0.75", "Shock Tanks stun reduce survivors speed to this amount.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);

	hWitchEnabled = CreateConVar("st_witch", "1", "Is Witch Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hWitchExtraHealth = CreateConVar("st_witch_extra_health", "0", "Witch Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hWitchFireImmunity = CreateConVar("st_witch_fire_immunity", "1", "Is Witch Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hWitchMaxWitches = CreateConVar("st_witch_max_witches", "30", "Maximum number of witches converted from common infected by the Witch Tank.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100.0);

	hShieldEnabled = CreateConVar("st_shield", "1", "Is Shield Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hCobaltExtraHealth = CreateConVar("st_cobalt_extra_health", "0", "Cobalt Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hCobaltFireImmunity = CreateConVar("st_cobalt_fire_immunity", "1", "Is Cobalt Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hCobaltSpecialSpeed = CreateConVar("st_cobalt_Special_speed", "2.5", "Cobalt Tanks movement value when speeding towards a survivor.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,1.0,true,5.0);

	hJumperEnabled = CreateConVar("st_jumper", "1", "Is Jumper Tank Enabled?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	hJumperExtraHealth = CreateConVar("st_jumper_extra_health", "0", "Jumper Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	hJumperSpeed = CreateConVar("st_jumper_speed", "1.20", "Jumper Tanks default movement speed.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,2.0);
	...
	hJumperFireImmunity = CreateConVar("st_jumper_fire_immunity", "1", "Is Jumper Tank immune to fire?",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1.0);
	...
	hGravityExtraHealth = CreateConVar("st_gravity_extra_health", "0", "Gravity Tanks receive this many additional hitpoints.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,100000.0);
	...
	hGravityPullForce = CreateConVar("st_gravity_pull_force", "-50.0", "Gravity Tanks pull force value. Higher negative values equals greater pull forces.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,-200.0,true,0.0);
