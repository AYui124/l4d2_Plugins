...
new bool:b_CommandEnabled;
new Handle:timerCommand;
...
public OnPluginStart() { 
	// Cvars
	hCvarAssaultReminderInterval = CreateConVar( "ai_assault_reminder_interval", "1", "Frequency(sec) at which the 'nb_assault' command is fired to make SI attack" );
	// Event hooks
...
public OnMapStart()
{
	timerCommand = INVALID_HANDLE;
}

public Action:L4D_OnFirstSurvivorLeftSafeArea(firstSurvivor) {
	b_CommandEnabled = true;
	if(timerCommand != INVALID_HANDLE)
	{
		KillTimer(timerCommand);
		timerCommand = INVALID_HANDLE;
	}
	timerCommand = CreateTimer( float(GetConVarInt(hCvarAssaultReminderInterval)), Timer_ForceInfectedAssault, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE );
}

public Action:Timer_ForceInfectedAssault( Handle:timer ) {
	if(b_CommandEnabled)
		return Plugin_Continue;
	if(CountSpecialInfectedBots()>0)
	{
