...
*   , smokers spawning in open view and boomers behind the air plane to
...
#define BLACKLIST_MAXWORDS	32
#define BLACKLIST_WORDSIZE	32
...
public OnPluginStart()
{
	...
	g_hMapBlacklist			= CreateConVar("l4d_fsb_mapblacklist"		,"c3m1_plankcountry,c5m5_bridge", "Sets which maps will not feature FSB. Comma (,) separated list",FCVAR_PLUGIN);
	...
	g_hHunterCanGhost 		= CreateConVar("l4d_fsb_hunter"				,"1"	,"Sets whether hunters are allowed to ghost",FCVAR_PLUGIN);
	g_hSpitterCanGhost 		= CreateConVar("l4d_fsb_spitter"			,"1"	,"Sets whether spitters are allowed to ghost",FCVAR_PLUGIN);
	g_hJockeyCanGhost 		= CreateConVar("l4d_fsb_jockey"				,"1"	,"Sets whether jockeys are allowed to ghost",FCVAR_PLUGIN);
	g_hChargerCanGhost 		= CreateConVar("l4d_fsb_charger"			,"1"	,"Sets whether chargers are allowed to ghost",FCVAR_PLUGIN);
	g_hQueueTimeout 		= CreateConVar("l4d_fsb_queuetimeout"		,"2.0"	,"If a player is still in queue to respawn as ghost, after this much time, ignore him and reset gamemode",FCVAR_PLUGIN);
...
public bool:OnClientConnect(client, String:rejectmsg[],maxlen)
{
	...
	if(StrContains(name, "smoker", false) == -1 && StrContains(name, "boomer", false) == -1 && StrContains(name, "hunter", false) == -1 && StrContains(name, "spitter", false) == -1 && StrContains(name, "jockey", false) == -1 && StrContains(name, "charger", false) == -1)
		return true;
...
NewMap()
{
	...
	{
		g_bIsVersus = true;
		if(StrContains(GameMode, "team", false) != -1)
		{
			g_bIsTeamVersus = true;
		}
		
		if(IsMapBlackListed())
		{
			g_bMapIsBlacklisted = true;
		}
	}
...
RoundReset()
{
	...
	if(!g_bIsVersus || g_bMapIsBlacklisted){return;}
...
bool:IsValidClass(client)
{
	...
	if(class == ZOMBIECLASS_SPITTER && GetConVarBool(g_hSpitterCanGhost))
		return true;
	
	if(class == ZOMBIECLASS_JOCKEY && GetConVarBool(g_hJockeyCanGhost))
		return true;
	
	if(class == ZOMBIECLASS_CHARGER && GetConVarBool(g_hChargerCanGhost))
		return true;
...
bool:IsMapBlackListed()
{
	decl String:sRawBlackList[BLACKLIST_MAXSIZE], String:sBlackList[BLACKLIST_MAXWORDS][BLACKLIST_WORDSIZE], String:MapName[BLACKLIST_WORDSIZE];
	
	GetCurrentMap(MapName,BLACKLIST_WORDSIZE);
	GetConVarString(g_hMapBlacklist,sRawBlackList,BLACKLIST_MAXSIZE);
	
	if(strlen(sRawBlackList) == 0){return false;}
	
	ExplodeString(sRawBlackList,",",sBlackList,BLACKLIST_MAXWORDS,BLACKLIST_WORDSIZE);
	
	for(new i=0;i<BLACKLIST_MAXWORDS;i++)
	{
		if(StrEqual(sBlackList[i],MapName,false)){return true;}
	}
